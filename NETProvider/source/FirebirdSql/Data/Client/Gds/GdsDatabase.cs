...
namespace FirebirdSql.Data.Client.Gds
{
	internal sealed class GdsDatabase : IDatabase
	{
		...
		public void CreateDatabase(DatabaseParameterBuffer dpb, string dataSource, int port, string database)
		{
			lock (this)
			{
				try
				{
					...
					try
					{
                        GdsGenericResponse response = (GdsGenericResponse)this.ReadResponse();
		...
		{
            IResponse response = this.connection.ReadResponse();

            if (response != null && response is GdsGenericResponse)
            {
                GdsGenericResponse genericResponse = (GdsGenericResponse)response;
