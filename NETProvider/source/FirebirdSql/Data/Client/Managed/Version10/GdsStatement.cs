...
namespace FirebirdSql.Data.Client.Gds
{
	internal class GdsStatement : StatementBase
	{
		...
		public override void Execute()
		{
			...
			lock (this.Database)
			{
				try
				{
					...
					if (this.Database.NextOperation() == IscCodes.op_sql_response)
					{
		...
		public override byte[] GetSqlInfo(byte[] items, int bufferLength)
		{
			lock (this.Database)
			{
				try
				{
                    ...
                    GenericResponse response = (GenericResponse)this.Database.ReadResponse();
        ...
        protected DbValue[] ReadStoredProcedureOutput()
		{
			try
			{
                IResponse response = this.Database.ReadResponse();
        ...
        protected DbValue[] ReadDataRow()
		{
			...
			lock (this.Database)
			{
				...
				for (int i = 0; i < this.fields.Count; i++)
				{
					try
					{
						value = this.Database.Receive.ReadValue(this.fields[i]);
