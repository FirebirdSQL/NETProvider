...
namespace FirebirdSql.Data.Embedded
{
	...
	public
#endif
	sealed class FesArray : ArrayBase
	{
		#region Fields
		...
		public override byte[] GetSlice(int sliceLength)
		{
			...
			int status = FbClient.isc_array_get_slice(
				statusVector,
				ref dbHandle,
				ref trHandle,
				ref this.handle,
		...
		public override void PutSlice(System.Array sourceArray, int sliceLength)
		{
			...
			IntPtr arrayDesc = marshaler.MarshalManagedToNative(
				this.Descriptor);
			...
			{
				buffer = this.encodeSlice(
					this.Descriptor, sourceArray, sliceLength);
			}

			int status = FbClient.isc_array_put_slice(
				statusVector,
				ref dbHandle,
				ref trHandle,
				ref this.handle,
		...
		protected override System.Array DecodeSlice(byte[] slice)
		{
			...
			// Create slice arrays
			sliceData = System.Array.CreateInstance(
				systemType, 
				lengths, 
				lowerBounds);

			System.Array tempData = System.Array.CreateInstance(
				systemType, sliceData.Length);

			// Infer data types
			type	= TypeHelper.GetFbType(this.Descriptor.DataType);
			dbType	= TypeHelper.GetDbDataType(
			...
			for (int i = 0; i < tempData.Length; i++)
			{
				...
				{							
					case DbDataType.Char:
						tempData.SetValue(encoding.GetString(
							slice, slicePosition, itemLength), i);
						break;
					...
					case DbDataType.Numeric:
					{
						...
						decimal dvalue = TypeDecoder.DecodeDecimal(
							evalue, 
							this.Descriptor.Scale,
							type);
		...
		private byte[] encodeSlice(ArrayDesc desc, Array sourceArray, int length)
		{
			...
			while (i.MoveNext())
			{
				switch (dbType)
				{
					...
					case DbDataType.Date:
						writer.Write(TypeEncoder.EncodeDate(Convert.ToDateTime(i.Current, CultureInfo.InvariantCulture.DateTimeFormat)));
						break;
					
					case DbDataType.Time:
						writer.Write(TypeEncoder.EncodeTime(Convert.ToDateTime(i.Current, CultureInfo.InvariantCulture.DateTimeFormat)));
						break;
