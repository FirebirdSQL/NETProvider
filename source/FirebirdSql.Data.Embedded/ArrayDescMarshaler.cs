...
namespace FirebirdSql.Data.Embedded
{
	internal class ArrayDescMarshaler
	{
		...
		/*
		public ArrayDesc MarshalNativeToManaged(IntPtr pNativeData)
		{
			// Obtain ArrayDescMarshal information
			ArrayDescMarshal descriptor = new ArrayDescMarshal();
			
			descriptor = (ArrayDescMarshal)Marshal.PtrToStructure(
				pNativeData,
				typeof(ArrayDescMarshal));

			// Create a new ArrayDesc
			ArrayDesc arrayDesc		= new ArrayDesc();
			arrayDesc.DataType		= descriptor.DataType;
			arrayDesc.Scale			= (short)descriptor.Scale;
			arrayDesc.Length		= descriptor.Length;
			arrayDesc.FieldName		= descriptor.FieldName;		
			arrayDesc.RelationName	= descriptor.RelationName;
			arrayDesc.Dimensions	= descriptor.Dimensions;
			...
			// Fill array bounds information
			arrayDesc.Bounds = new ArrayBound[16];

			for (int i = 0; i < arrayDesc.Dimensions; i++)
			{
				// Obtain array bound
				ArrayBoundMarshal arrayBound = new ArrayBoundMarshal();
				 
				arrayBound = (ArrayBoundMarshal)Marshal.PtrToStructure(
					(IntPtr)(pNativeData.ToInt32() + ArrayDescMarshal.ComputeLength(i)),
					typeof(ArrayBoundMarshal));

				arrayDesc.Bounds[i].LowerBound = arrayBound.LowerBound;
				arrayDesc.Bounds[i].UpperBound = arrayBound.UpperBound;
			}
