...
namespace FirebirdSql.Data.Firebird.Tests
{
	...
	public class FbDataAdapterTest : BaseTest 
	{
		...
		[Test]
		public void InsertWithDBNull()
		{
			FbTransaction	transaction = this.Connection.BeginTransaction();
			FbCommand		command		= new FbCommand("select int_field, smallint_field from TEST", Connection, transaction);
			FbDataAdapter	adapter		= new FbDataAdapter(command);
			...
			newRow["int_field"]			= 1010;
			newRow["smallint_field"]	= DBNull.Value;
		...
		[Test]
		public void ExecuteProcedureFill()
		{
			...
			Assert.AreEqual(1, ds.Tables[0].Rows.Count, "Incorrect row count.");
