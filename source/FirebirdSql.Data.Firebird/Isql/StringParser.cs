...
namespace FirebirdSql.Data.Firebird.Isql
{
	...
	public class StringParser 
	{
		...
		/// without having to initializate the target string on <see cref="Parse"/> method. See the example for further details.
		/// </remarks>
		...
		public int Parse(string targetString, int start) 
		{
			...
			{
				if (string.Compare(this.source[i].ToString(), this.token[0].ToString(), !this.caseSensitive) == 0) 
				{
					if (string.Compare(this.source.Substring(i, this.tokenLength), this.token, !this.caseSensitive) == 0) 
					{
						i += this.tokenLength; 
						break;
					}
		...
		/// <summary>
		/// <para>Repeats the parsing starting on the index returned by <see cref="Parse"/> method.</para>
		/// You can also call <b>ParseNext</b> directly (without calling <see cref="Parse"/>) if you define the text to be parsed at instance construction.  
		/// </summary>
		/// <returns>The index of the char next char after the <see cref="Token"/> end.</returns>
		/// <remarks>If nothing is parsed the method will return -1. Case the <see cref="Token"/> wasn't found until the end of the string the method returns 
		/// (in <see cref="Result"/>) the string found between the starting index and the end of the string.</remarks>
		...
		public int IndexOf(string substring, int startIndex)
		{
			...
			while (i < this.sourceLength) 
			{
				if (String.Compare(this.source[i].ToString(), substring[0].ToString(), !this.caseSensitive) == 0)
				{
					...
					{
						if (String.Compare(this.source[j].ToString(), substring[j-i].ToString(), !this.caseSensitive) == 0)
							j++;
						else 
							break;
					}
		...
		public static bool StartsWith(string source, string token, bool ignoreCase) 
		{
			...
			return string.Compare(token, source.Substring(0, token.Length), ignoreCase) == 0;
		}
