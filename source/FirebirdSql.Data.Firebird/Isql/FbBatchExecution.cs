...
namespace FirebirdSql.Data.Firebird.Isql
{
	...
	public class FbBatchExecution
	{
		...
		private AttachmentParams	attachParams	= null;
		private FbCommand			sqlCommand		= null;

		// control fields
		private bool requiresNewConnection = true;
		...
		public FbBatchExecution()
		{
			...
			this.attachParams	= new AttachmentParams();
		}
		...
		public FbBatchExecution(FbConnection sqlConnection)
		{
			...
			else
			{
				...
				this.attachParams	= new AttachmentParams(sqlConnection.ConnectionString);
		...
		public virtual void Execute() 
		{
			...
			foreach (string sqlStatement in this.SqlStatements)
			{
				...
				catch (Exception ex)
				{
					...
					throw new FbException(String.Format("An exception was thrown when executing command: {0}\nBatch execution aborted\nThe returned message was: {1}", sqlStatement, ex.Message));
				}
			}
		...
		protected internal void ConnectToDatabase(string connectDbStatement)
		{
			...
			parser.Token = " ";
			parser.ParseNext();
			if (parser.Result.Trim().ToUpper() != "CONNECT")
			{
				throw new Exception("Malformed isql CONNECT statement. Expected keyword CONNECT but something else was found.");
			}
			parser.ParseNext();
			this.attachParams.Database = parser.Result.Replace("'", "");
			while (parser.ParseNext() != -1)
			{
				switch (parser.Result.Trim().ToUpper())
				{
					case "USER":
						parser.ParseNext();
						this.attachParams.UserName = parser.Result.Replace("'", "");
						break;

					case "PASSWORD":
						parser.ParseNext();
						this.attachParams.UserPassword = parser.Result.Replace("'", "");
						break;
					...
					case "ROLE":
						parser.ParseNext();
						this.attachParams.Role = parser.Result.Replace("'", "");
						break;
		...
		protected internal FbConnection ProvideConnection()
		{
			if (requiresNewConnection)
			{
				...
				this.sqlConnection = new FbConnection(this.attachParams.ToString());	
			}
		...
		*/
		...
		{
			char type = sqlStatement == null ? ' ' : sqlStatement.Trim().ToUpper()[0];
