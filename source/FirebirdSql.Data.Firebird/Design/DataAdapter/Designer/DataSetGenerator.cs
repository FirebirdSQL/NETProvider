...
namespace FirebirdSql.Data.Firebird.Design
{
	internal class DataSetGenerator
	{
			...
			foreach (string tableName in tables)
			{
				...
				for (int i = 0; i < rows.Count; )
				{
					ForeignKey fk = new ForeignKey(
						FormatIdentifier(rows[i]["fk_name"], properCase),
						FormatIdentifier(rows[i]["pk_table_name"], properCase),
						FormatIdentifier(rows[i]["fk_table_name"], properCase),
						FormatIdentifier(rows[i]["update_rule"], properCase),
						FormatIdentifier(rows[i]["delete_rule"], properCase));

					string fkName = (string)rows[i]["fk_name"];

					while (i < rows.Count && fkName == (string)rows[i]["fk_name"])
					{
						fk.PkColumns.Add(FormatIdentifier(rows[i]["pk_column_name"], properCase));
						fk.FkColumns.Add(FormatIdentifier(rows[i]["fk_column_name"], properCase));
		...
		private static string FormatIdentifier(string value, bool properCase)
		{
			if (properCase)
			{
				string result = CultureInfo.CurrentCulture.TextInfo.ToTitleCase(value.ToLower());
