...
namespace FirebirdSql.Data.Firebird
{
	...
	public sealed class FbCommandBuilder : Component
	{
		...
		public static void DeriveParameters(FbCommand command)
		{
			...
			foreach (DataRow row in spSchema.Rows)
			{
                dataTypes.RowFilter = String.Format(
                    "TypeName = '{0}'", row["PARAMETER_DATA_TYPE"]);
                dataTypes.RowStateFilter = DataViewRowState.CurrentRows;
				...
				parameter.Size = Convert.ToInt32(row["PARAMETER_SIZE"], CultureInfo.InvariantCulture.NumberFormat);
		...
		private FbCommand BuildInsertCommand(
			DataRow				row, 
		...
		private FbCommand BuildUpdateCommand(DataRow row, DataTableMapping tableMapping)
		{
			...
			{				
				if (this.IncludedInWhereClause(schemaRow))
				{
					...
					{
						where.AppendFormat(whereClausule2, quotedId, parameter.ParameterName);
					}
					else
					{
		...
		{
			FbParameter parameter = new FbParameter(String.Format("@p{0}", index + 1), (FbDbType)schemaRow["ProviderType"]);

			parameter.Size = Convert.ToInt32(schemaRow["ColumnSize"], CultureInfo.InvariantCulture.NumberFormat);
			if (schemaRow["NumericPrecision"] != DBNull.Value)
			{
				parameter.Precision	= Convert.ToByte(schemaRow["NumericPrecision"], CultureInfo.InvariantCulture.NumberFormat);
			}
			if (schemaRow["NumericScale"] != DBNull.Value)
			{
				parameter.Scale	= Convert.ToByte(schemaRow["NumericScale"], CultureInfo.InvariantCulture.NumberFormat);
			}
			parameter.SourceColumn	= schemaRow["BaseColumnName"].ToString();
			parameter.IsNullable	= Convert.ToBoolean(schemaRow["AllowDbNull"]);
