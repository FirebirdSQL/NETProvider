...
namespace FirebirdSql.Data.Firebird.Services
{
	...
	{				
		...
		public void AddUser(FbUserData user)
		{
			...
			{
				this.StartSpb.Append(
					IscCodes.isc_spb_sec_firstname, 
					user.FirstName);
			}
			if (user.MiddleName != null && user.MiddleName.Length > 0)
			{
				this.StartSpb.Append(
					IscCodes.isc_spb_sec_middlename, 
					user.MiddleName);
			}
			if (user.LastName != null && user.LastName.Length > 0)
			{
				this.StartSpb.Append(
					IscCodes.isc_spb_sec_lastname, 
					user.LastName);
			}
			if (user.UserID != 0)
			{
				this.StartSpb.Append(
					IscCodes.isc_spb_sec_userid, 
					user.UserID);
			}
			if (user.GroupID != 0)
			{
				this.StartSpb.Append(
					IscCodes.isc_spb_sec_groupid, 
					user.GroupID);
			}
			...
			{
				this.StartSpb.Append(
					IscCodes.isc_spb_sql_role_name, 
					user.RoleName);
			}
