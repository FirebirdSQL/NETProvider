...
namespace FirebirdSql.Data.Firebird.Services
{
	...
	public sealed class FbConfiguration : FbService
	{
		...
		public void SetSqlDialect(int sqlDialect)
		{
			...
			this.StartSpb.Append(
				IscCodes.isc_spb_prp_set_sql_dialect, 
				sqlDialect);
		...
		public void SetSweepInterval(int sweepInterval)
		{
			...
			this.StartSpb.Append(
				...
				this.Parameters.Database);
			this.StartSpb.Append(
				IscCodes.isc_spb_prp_sweep_interval, 
				sweepInterval);
		...
		public void SetPageBuffers(int pageBuffers)
		{
			...
			this.StartSpb.Append(
				...
				this.Parameters.Database);
			this.StartSpb.Append(
				IscCodes.isc_spb_prp_page_buffers, 
				pageBuffers);
		...
		public void DatabaseShutdown(FbShutdownMode mode, int seconds)
		{
			...
			switch (mode)
			{
				case FbShutdownMode.Forced:
					this.StartSpb.Append(
						IscCodes.isc_spb_prp_shutdown_db, 
						seconds);
					break;
				...
				case FbShutdownMode.DenyConnection:
					this.StartSpb.Append(
						IscCodes.isc_spb_prp_deny_new_attachments, 
						seconds);
					break;
			}
		...
		public void DatabaseOnline()
		{
			...
			this.StartSpb.Append(
				...
				IscCodes.isc_spb_prp_db_online);
		...
		public void SetForcedWrites(bool forcedWrites)
		{
			...
			{
				this.StartSpb.Append(
					IscCodes.isc_spb_prp_write_mode, 
					(byte)IscCodes.isc_spb_prp_wm_sync);
			}
			else
			{
				this.StartSpb.Append(
					IscCodes.isc_spb_prp_write_mode, 
					(byte)IscCodes.isc_spb_prp_wm_async);
			}			
		...
		public void SetReserveSpace(bool reserveSpace)
		{
			...
			if (reserveSpace)
			{
				this.StartSpb.Append(
					...
					(byte)IscCodes.isc_spb_prp_res);
			}
			else
			{
				this.StartSpb.Append(
					IscCodes.isc_spb_prp_reserve_space, 
					(byte)IscCodes.isc_spb_prp_res_use_full);
			}
		...
		public void SetAccessMode(bool readOnly)
		{
			...
			{
				this.StartSpb.Append(
					IscCodes.isc_spb_prp_access_mode, 
					(byte)IscCodes.isc_spb_prp_am_readonly);
			}
			else
			{
				this.StartSpb.Append(
					IscCodes.isc_spb_prp_access_mode, 
					(byte)IscCodes.isc_spb_prp_am_readwrite);
			}
