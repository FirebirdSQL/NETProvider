...
namespace FirebirdSql.Data.Design.Parameters
{
	internal class FbParameterConverter : TypeConverter
	{
			...
			CultureInfo culture, object value, Type destinationType) 
		{
			...
			if (destinationType == typeof(InstanceDescriptor) && value is FbParameter) 
			{
				FbParameter param = (FbParameter)value;

				if (param.ParameterName != null && param.ParameterName.Length > 0)
				{
				...
				if (param.SourceColumn != null && param.SourceColumn.Length > 0)
				{
					sourceColumnChanged = true;
				}
				if (param.Direction != ParameterDirection.Input	|| param.Precision != 0	|| param.Scale != 0	||
					param.IsNullable != false || param.SourceVersion	!= DataRowVersion.Current)
				{
					miscChanged = true;
				}
				
				if (parameterNameChanged && valueChanged && !sizeChanged && !fbDbTypeChanged &&
					!sourceColumnChanged && !miscChanged)
				{
					ctorTypes = new Type[] {
											   typeof(string)	, 
											   typeof(object) 
				...
				else if (parameterNameChanged && !sizeChanged && !sourceColumnChanged && !valueChanged && !miscChanged)
				{
					ctorTypes = new Type[] {
											   typeof(string)	, 
											   typeof(FbDbType)
				...
				ConstructorInfo ctor = typeof(FbParameter).GetConstructor(ctorTypes);
