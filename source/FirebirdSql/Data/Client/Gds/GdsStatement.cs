...
namespace FirebirdSql.Data.Client.Gds
{
	internal class GdsStatement : StatementBase
	{
		...
		public override void Prepare(string commandText)
		{
			...
			lock (this.db)
			{
				...
				try
				{
                    ...
                    GdsGenericResponse response = (GdsGenericResponse)this.db.ReadResponse();
				...
				lock (this.db)
				{
					try
					{
						...
						if (this.db.NextOperation() == IscCodes.op_fetch_response)
						{
							...
							{
                                response = this.db.ReadResponse();

                                if (response is GdsFetchResponse)
                                {
                                    GdsFetchResponse fetchResponse = (GdsFetchResponse)response;
        ...
        private DbValue[] ReadStoredProcedureOutput()
		{
			try
			{
                IResponse response = this.db.ReadResponse();

				if (response is GdsSqlResponse)
				{
					if (((GdsSqlResponse)response).Count > 0)
					{
