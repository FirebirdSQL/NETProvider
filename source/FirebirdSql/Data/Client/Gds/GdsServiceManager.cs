...
namespace FirebirdSql.Data.Client.Gds
{
	internal sealed class GdsServiceManager : IServiceManager
	{
		...
		{
            GdsGenericResponse response = null;
			...
			lock (this)
			{
				try
				{
                    ...
                    response = (GdsGenericResponse)this.connection.ReadResponse();
			...
			byte[]					buffer)
		{
			lock (this)
			{
				try
				{
					...
					GdsGenericResponse response = (GdsGenericResponse)this.connection.ReadResponse();
