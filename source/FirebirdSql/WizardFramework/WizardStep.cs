...
namespace FirebirdSql.WizardFramework
{
	...
	internal class WizardStep : UserControl
	{
		...
		private List<WizardStep> backSteps;
        private List<WizardStep> nextSteps;
		...
		[Browsable(false)]
		public List<WizardStep> NextSteps
		{
			get
			{
				if (this.nextSteps == null)
				{
					this.nextSteps = new List<WizardStep>();
				}
		...
		/// <remarks>This collection handles the steps we can navigate to througth the next	buttong	of a Wizard.</remarks>
		[Browsable(false)]
		public List<WizardStep> BackSteps
		{
			get
			{
				if (this.backSteps == null)
				{
                    this.backSteps = new List<WizardStep>();
				}
