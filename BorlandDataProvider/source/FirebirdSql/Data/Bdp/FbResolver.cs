...
namespace FirebirdSql.Data.Bdp
{
	public class FbResolver : ISQLResolver
	{
        ...
        // Added by RPH for Borland 2006.  Note that only the primary key should
        // ever be passed into this routine even though it will take additional
        // where parameters.
			...
			string				tableName)
		{
			...
			if (this.CheckColumns(columns))
			{
				...
				// Build list of columns to select and the where clause
				foreach (DataRow schemaRow in columns)
				{
					if (sql.Length > 0)         // Build columns to select
					{
						sql.Append(", ");
					}
					sql.AppendFormat("{0}{1}{2}", this.QuotePrefix,
									schemaRow["ColumnName"], this.QuoteSuffix);
					...
					{                           // Add the where clause
						...
						// Add where clause for this field
				...
				sql.AppendFormat(" FROM {0} WHERE {1}",
				  this.GetQuotedIdentifier(tableName), where.ToString());
				sql.Insert(0, "SELECT ");
			}

			return sql.ToString();      // Return result
		}
			...
			if (this.CheckColumns(columns))
			{
				...
				foreach (DataRow schemaRow in columns)
				{
					if (this.IsUpdatable(schemaRow))
					{
						if (fields.Length > 0)
						{
							fields.AppendFormat("{0} ", this.separator);
						}
						if (values.Length > 0)
						{
							values.AppendFormat("{0} ", this.separator);
						}
