...
namespace FirebirdSql.Data.Bdp
{
	public class FbCursor : ISQLCursor
	{
		...
		public int GetBlob(short index, ref byte[] buffer, ref int nullInd, int length)
		{
			this.CheckPosition();
			this.CheckIndex(index);
		...
		public int GetVarBytes(short index, ref byte[] buffer, ref int nullInd, int length)
		{
		...
		public int GetDecimalAsString(short index, StringBuilder data, ref int nullInd)
		{
			this.CheckPosition();
			this.CheckIndex(index);
